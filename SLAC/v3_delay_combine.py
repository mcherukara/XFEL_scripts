...
#Scan 2
a1=tif.imread(sys.argv[4])
b1=float(sys.argv[5]) #Step size
c1=float(sys.argv[6]) #Init time
...
data1=np.zeros((dim[0]+dim1[0],dim[1],3),float)
data2=np.zeros((dim[0]+dim1[0],dim[2],3),float)
...
coms=np.zeros((dim[0]+dim1[0],3),float) #Stores time,  COMx and COMy of each frame
...
for fr in range(dim[0]+dim1[0]):
  ...
  if(fr<dim[0]):
   ...
   data1[fr,:,0],data2[fr,:,0]=(fr-dim[0])*b1+c1,(fr-dim[0])*b1+c1
   print (fr-dim[0])*b1+c1
  if(fr<dim[0]):
   ...
   data1[fr,:,2],data2[fr,:,2]=np.mean(a1[fr-dim[0],:,:], 1),np.mean(a1[fr-dim[0],:,:], 0) #Colour map
   coms[fr,1:3]=np.array(ndimage.measurements.center_of_mass(a1[fr-dim[0],:,:]))-np.array(ndimage.measurements.center_of_mass(a[0,:,:]))
   try:
    popt, pcov = curve_fit(twoD_Gaussian, (x,y), a1[fr-dim[0],:,:].ravel(), p0=initial_guess, maxfev=10000)
   except:
    print fr, "failed"
    popt=temp_popt
   temp_popt=popt 
...
plt1.pcolormesh(data2[:,:,0],data2[:,:,1],data2[:,:,2],shading='gouraud',vmax=data2[:,:,2].max()/1.0)
plt2.pcolormesh(data1[:,:,0],data1[:,:,1],data1[:,:,2],shading='gouraud',vmax=data2[:,:,2].max()/1.0)
